...
class User{
	...
	public static function findByEmail($email){
        ...
        public static function existsAndReg($email){

                // Does the user exist in the database?
                $result = ORM::for_table('reg_users')
                                           ->where('email', $email)
                                           ->where_not_equal('full_name','')
	...
	/**
	 * Create a new user and save it to the database
	 * @param string $email The user's email addrese
	 * @return User
	 */

	public static function createFull($email,$phone,$full_name,$landing){

		// Write a new result to the database and return it
		$result = ORM::for_table('reg_users')->where('email', $email)->find_one();
               if($result){
		$result->phone = $phone;
        ...
        private static function update($email,$phone,$full_name){
	...
	public function generateToken(){
		...
		// and mark it as valid for 48 hours only

		$this->orm->set('token', $token);
		// PostgreSQL equivalent of ADDTIME(NOW(),'48:00')
		$this->orm->set_expr('token_validity', "NOW() + INTERVAL '48 hours'");
		$this->orm->save();
